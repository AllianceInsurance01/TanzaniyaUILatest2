...
@Component({
  selector: 'app-survivallist-details',
  templateUrl: './survivallist.component.html',
  styleUrls: ['./survivallist.component.scss']
})
export class SurvivalDetailsComponent implements OnInit {

  activeMenu = "SURVIVAL";insuranceName:any;insuranceId:any;productId:any;loginId:any;
 ...
 this.taxList.push();
    ...
    this.sharedService.onPostMethodSync(urlLink, ReqObj).subscribe(
      (data: any) => {
        ...
        //   this.TaxFor=data?.Result?.TaxFor;
            this.taxList = data?.Result?.SurvivalBenefit;
            console.log("TaxList ",this.taxList);
            this.gettotal(this.taxList);
  ...
  gettotal(taxlist){
    this.totalAmount=0;let total
    let i=0;
    for(let f of taxlist){
      total=f.SurrenderPercentage;
       this.totalAmount = Number(total)+Number(this.totalAmount);
       i+=1;
    }
    console.log('Change',this.totalAmount);

  }

   gettotalchange(taxlist){
 this.totalAmount = Number(taxlist)+Number(this.totalAmount);
   console.log('Change',this.totalAmount);
  }
  onStartDateChange(){
  ...
  "ProductId":this.productId,
  "SectionId": "99999",
  "SurvivalBenefit":this.taxList,
  "SaveType":type

    }
    let urlLink = `${this.CommonApiUrl1}master/insertsurvival`;
